...
from .model import stCOMETEncoder, stCOMETSparseEncoder, stCOMETMultiHeadEnhancer
...
class stCOMET:
    ...
    def _is_sparse_datatype(self):
        return self.datatype in ['Stereo', 'Slide']
    ...
    def _prepare_anndata_inputs(self, original_adata):
        ...
        if 'adj' not in original_adata.obsm.keys():
            if self._is_sparse_datatype():
                construct_stcomet_knn_graph(self.adata)
            else:
    ...
    def _prepare_training_adjacency(self):
        if self._is_sparse_datatype():
            ...
            self.adj = preprocess_stcomet_sparse_adjacency(self.adj).to(self.device)
        else:
            self.adj = preprocess_stcomet_adjacency(self.adj)
            self.adj = torch.FloatTensor(self.adj).to(self.device)

    def _stcomet_neighbor_contrastive_loss(self, z1, z2, adj, tau=0.5, hidden_norm=True):
    ...
    def _stcomet_contrastive_loss(self, z1, z2, adj, tau=1.0, hidden_norm=True):
        l1 = self._stcomet_neighbor_contrastive_loss(z1, z2, adj, tau, hidden_norm)
        l2 = self._stcomet_neighbor_contrastive_loss(z2, z1, adj, tau, hidden_norm)
        return (l1 + l2) * 0.5
    ...
    def _prepare_training_graph_state(self):
        if not self._is_sparse_datatype():
    ...
    def _build_model(self):
        if self._is_sparse_datatype():
            return stCOMETSparseEncoder(self.dim_input, self.dim_output, self.graph_neigh).to(self.device)
    ...
    def _run_dense_training_epoch(self):
        ...
        if len(embeddings) > 1:
            ...
            for i in range(1, len(embeddings)):
                contrastive_loss_value += self._stcomet_contrastive_loss(
                    base_embedding,
                    embeddings[i],
                    self.graph_neigh,
                    0.25,
        ...
        if hasattr(self, 'recon_weight') and hasattr(self, 'contrastive_weight'):
            return self.recon_weight * loss_feat + self.contrastive_weight * contrastive_loss_value
        return 0.5 * loss_feat + contrastive_loss_value

    def _run_sparse_training_epoch(self):
        _, emb, _, _ = self.model(self.features, self.features_a, self.adj)
        loss_feat = F.mse_loss(self.features, emb)
        if hasattr(self, 'recon_weight'):
            return self.recon_weight * self.alpha * loss_feat
        return self.alpha * loss_feat
    ...
    def _get_current_embedding(self):
        if self._is_sparse_datatype():
            return self.model(self.features, self.features_a, self.adj)[1]
    ...
    def _train_stcomet_with_callback(self, callback=None):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            if self._is_sparse_datatype():
                loss = self._run_sparse_training_epoch()
            else:
